...
$(document).ready(function() {
    $('#leaderboard-wrapper').append(leaderboards[globals.handlebars]({leaderboards: globals.leaderboard, type: globals.type.replace('s1_', '').replace('s2_', '').replace('s3_', '').replace('s4_', '').replace('s5_', '').replace('s6_', '').replace('first_place', 'First Place'), 'season': globals.season}));
